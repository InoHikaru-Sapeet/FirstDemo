...
⚠️ **§5.2 との差分が1件だけある**（2026-08-16 の決定2）:
`tunable_thresholds.dedup.monthly_lookback_months`（＝3）。§11.1 の月次
「直近数ヶ月」の月数を config で持つと決めたため足した鍵で、**仕様書 §5.2 側の
改訂は T-38**（TASKS.md T-21 備考・要確認事項 #10 に記録済み）。これ以外の値を
「逐語コピー」から動かさないこと。
"""
