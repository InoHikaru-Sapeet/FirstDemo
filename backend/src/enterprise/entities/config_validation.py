...
class ConfigIssueCode(StrEnum):
    ...
    **同じ方法で** `path` からフォーム欄へマッピングできるようにするため。
    ...
    # ⚠️ **保存の可否ではない。** ここに挙がった項目も `PUT /config` では保存
    # できる（編集可能パラメータであることに変わりはない）。言っているのは
    # 「**この変更の結果を、保存済みの採点結果から試算できない**」ということ。
    # 変更しても採点をやり直さないと結果が分からない（軸の配点・対象業界など）
    RESCORE_REQUIRED = "rescore_required"
    # 試算に要るデータが成果物に残っていない、または採否の件数に効かない
    # （除外ルールの当たり判定・重複判定の入力・生成/描画側のパラメータ）
